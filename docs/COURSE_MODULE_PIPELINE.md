...
- `revision-book`: flexible 10-100 question prediction set generated from topic frequency, recency, gaps, marks, harder-question signals, and student progress. It must be blueprint-first: cover as many topics as the target allows before repeating any topic, then repeat in interleaved rounds when a course has fewer topics than the chosen length. Random Mock uses the same teacher-facing topic-mix idea: selected topic chips split the target count evenly first, then questions are shuffled within each selected topic.
