(function () {
  ...
  const NOTES = (window.ELITE_IAL_NOTES || {}).wma11 || { topics: [] };
  const SOLVED_KEY = "eliteWMA11SolvedV1";
  const MISTAKE_KEY = "eliteWMA11MistakeBoxV1";
  const TRAINER_KEY = "eliteWMA11FinalTrainerV1";
  const state = {
    filtered: [],
    activeIndex: 0,
    showSolution: false,
  ...
  function notePracticeHref(slug) {
    return `ial/wma11/index.html?topic=${encodeURIComponent(slug)}#ialFilters`;
  }
  ...
  function renderNotes() {
    if (els.notesBooklet && NOTES.booklet) {
      els.notesBooklet.innerHTML = `
        <div>
          <span class="eyebrow">${escapeHtml(NOTES.code || "WMA11")} strategy route</span>
        ...
        <div class="ial-note-feature-actions">
          ...
          <a class="button light" href="ial/wma11/index.html#ialFilters">Open classified</a>
        </div>
      `;
    }
