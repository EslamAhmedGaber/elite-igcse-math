(function () {
  ...
  function paperSlug(item) {
    return `WMA12_${item.year}_${item.session}`;
  }
  ...
  function renderQuestion() {
    ...
    if (!item) {
      els.stage.innerHTML = '<div class="ial-empty">No WMA12 questions match these filters yet.</div>';
      return;
    }
  ...
  function printMock(includeSolutions = false) {
    ...
    printArea.innerHTML = state.mock.map((item, index) => `
      <article class="ial-print-page">
        <h2>WMA12 Pure 2 Mock - Question ${index + 1}</h2>
