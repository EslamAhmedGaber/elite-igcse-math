...
OUT_PATH = ROOT / "private_output" / "wma12_source_audit.json"

QP_RE = re.compile(r"IAL_MATHS_(\d{4})_([A-Za-z]+)_P2(R?)_QP\.pdf$", re.IGNORECASE)
MS_RE = re.compile(r"IAL_MATHS_(\d{4})_([A-Za-z]+)_P2(R?)_MS\.pdf$", re.IGNORECASE)
...
def paper_slug(year: str, session: str, variant: str) -> str:
...
def main() -> int:
    ...
    for path in sorted(SOURCE_DIR.glob("IAL_MATHS_*_P2*_*.pdf")):
