...
BANK_PATH = ROOT / "private_output" / "wme01_full_bank.json"
HTML_ROOT = ROOT / "private_output" / "wme01_books_html"
PAPER_HTML_ROOT = HTML_ROOT / "Papers"
OUT = ROOT / "downloads" / "IAL" / "WME01"
...
def load_items() -> list[dict]:
    ...
    if len(bank.get("questions", [])) != 138:
        raise SystemExit(f"Expected 138 WME01 questions, found {len(bank.get('questions', []))}")
...
def paper_slug(item: dict) -> str:
    return f"WME01_{int(item['year'])}_{item['session']}"
...
def cover(title: str, subtitle: str, count: int, answer_mode: bool, count_label: str = "topic placements", badge: str | None = None) -> str:
  ...
  <div class="cover-grid">
    ...
    <div><strong>WME01</strong><span>Mechanics 1</span></div>
...
def build_topic_html(filename: str, title: str, subtitle: str, items: list[dict], answer_mode: bool) -> tuple[Path, Path]:
    ...
    html_path.write_text(html_doc(title, "wme01_books.css", chunks), encoding="utf-8")
...
.cover:before { content:"M1"; position:absolute; right:7mm; bottom:5mm; color:rgba(22,27,46,.065); font-size:158pt; font-weight:800; }
...
"""
    ...
    (HTML_ROOT / "wme01_books.css").write_text(css, encoding="utf-8")
...
def print_pdf(html_path: Path, pdf_path: Path) -> None:
    ...
    if not edge.exists():
        raise SystemExit("Microsoft Edge is required to print WME01 PDFs on this machine.")
    if pdf_path.exists():
        pdf_path.unlink()
    subprocess.run(
        [
...
def main() -> None:
    ...
    outputs = [
        build_topic_html(
            "WME01_Classified_Questions.html",
            "WME01 Mechanics 1 Classified Questions",
            "138 unique questions grouped by primary topic, with cross-topic placements where a question also belongs in another chapter.",
            classified_placements,
            False,
        ),
        build_topic_html(
            "WME01_Expertise_Questions.html",
            "WME01 Mechanics 1 Expertise Questions",
            "Questions 6 and above, grouped by topic with cross-topic placements where useful.",
            expertise_placements,
            False,
        ),
        build_topic_html(
            "WME01_Classified_With_Answers.html",
            "WME01 Mechanics 1 Classified With Answers",
            "Each placement has the question followed by a worked-solution page in Dr Eslam Ahmed's solution style.",
            classified_placements,
            True,
        ),
        build_topic_html(
            "WME01_Expertise_With_Answers.html",
            "WME01 Mechanics 1 Expertise With Answers",
            "Questions 6 and above with worked-solution pages and cross-topic placements.",
            expertise_placements,
            True,
        ),
        build_all_papers_html(
            "WME01_Past_Paper_Solutions.html",
            "WME01 Mechanics 1 Past Paper Solutions",
            "The full WME01 bank rebuilt in original paper order, with each question followed by its worked solution.",
            items,
        ),
    ]
